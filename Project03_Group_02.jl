...
begin
    m1 = 0.1  # Mass of the block [kg]
    m2 = 0.3  # Mass of the bar [kg] (feels very heavy)
    L  = 0.4  # Length of the bar [m] (feels long)
    k  = 10.0 # Spring constant [N/m] (storgner spring could address the heavy mass)
...
# ╔═╡ 94df80cc-fcd6-48f0-be92-5016f8aded75
begin
	function constraint_forces_physical(q, qdot)
	...
	function get_reaction_forces(q, qdot)
	    λ, Qc = constraint_forces_physical(q, qdot)
	
	    # Normal force on block (y-direction)
	    N = Qc[2]
...
# ╔═╡ 329940e0-a1fd-46e2-8ddb-f409c7cdf951
md"""## 2. Graphs and Solutions
"""
...
# ╔═╡ d255d911-e1d4-4265-9ed7-38608d8947dd
begin
    # --- Time ---
    t = solution.t

    # --- Beam tip velocity ---
    tip_vx = diff(beam_tip_x) ./ diff(t)
    tip_vy = diff(beam_tip_y) ./ diff(t)

    # --- Speed magnitude ---
    tip_speed = sqrt.(tip_vx.^2 .+ tip_vy.^2)

    # --- Plot colored line ---
    plot(
        beam_tip_x,
        beam_tip_y,
        line_z = [tip_speed; tip_speed[end]],
        linewidth = 4,
        xlabel = "Beam Tip X Position (m)",
        ylabel = "Beam Tip Y Position (m)",
        title = "Beam Tip Path Colored by Speed",
        colorbar = true,
        aspect_ratio = :equal,
        legend = false
    )
...
# ╔═╡ d39fc2c8-22a5-4d6f-9be6-9549a67fd34e
x = solution[1, :]   # cart displacement

# ╔═╡ 783c1753-e05d-44cb-b131-e1948332d70e
F_spring = -k .* x   # spring force

# ╔═╡ 1f6a658f-5079-405f-a9e9-1fa9ac16e719
x_dot = solution[7, :]   # cart velocity

# ╔═╡ 04422d5d-9fdc-44f8-a693-4540b2baca81
theta_dot = solution[12, :]  # beam angular velocity
...
# ╔═╡ a41a37e9-832d-4e60-ab08-3f8574c3d6b0
...
# ╔═╡ a799deb9-e2ae-4098-b758-845507dc6a1c
begin
	# --- Plot hinge reaction forces ---
	plot(time_elapsed, hinge_force_x,
	     xlabel="Time (s)",
	     ylabel="Force (N)",
	     title="Hinge Reaction Forces",
	     label = "Horizontal Reaction Force")
		
	plot!(time_elapsed, hinge_force_y,
	     xlabel="Time (s)",
	     ylabel="Force (N)",
		 label = "Vertical Reaction Force",
...
# ╔═╡ 2a656d52-4d4f-4291-8a38-7de094625a80
md"""## 3. Animation Visualization
"""
...
begin
	...
	animation_system_visualization = @animate for index in 1:5:length(time_elapsed)
...
gif(animation_system_visualization, "cart_beam_animation.gif", fps=1200)

# ╔═╡ 0db6940b-ca6b-4ecd-ba17-56cfd8350cd0
begin
	animation_key_location_trails = @animate for index in 1:5:length(time_elapsed)
		#Defines cart geometry
 		cart_x, cart_y = cart_vertices(
            cart_position_x[index],
            cart_position_y[index],
...
gif(animation_key_location_trails, "cart_beam_animation_with_trails.gif", fps=1200)

# ╔═╡ 08f8315b-ae0d-4dbe-8aea-7558b2b1d0d5
begin
	# Peak magnitudes over the full simulation
	peak_ground_force = maximum(abs.(cart_vertical_force))
	peak_hinge_fx     = maximum(abs.(hinge_force_x))
	peak_hinge_fy     = maximum(abs.(hinge_force_y))	
end

# ╔═╡ dbcd1aeb-d9de-4d46-b35c-4f9d4568eee1
begin
	animation_force_display = @animate for index in 1:2:length(time_elapsed)
		#Plot the reaction forces on the blocks
		force_scale_v = 0.03
		force_scale_h = 0.08
    	...
    	plot!(
    		...
    		linecolor = :red,
    		linewidth = 4,
    		label = "X Hinge Reaction"
    	)
    	plot!(
       		[NaN], [NaN],
       		linecolor = :orange,
       		linewidth = 4,
       		label = "Y Hinge Reaction"
    	)
		
		#plots the beam

		force_min = minimum(cart_vertical_force.*force_scale_v)-L/6
		force_max =  maximum(hinge_force_y.*force_scale_v)+L/6
		...
		plot!(
	        ...
	        label= false,
	        xlims=(x_min, x_max),
	        ylims=(force_min,force_max),
	        xlabel="X Position (m)",
	        ylabel="Y Position (m)",
	        title="Reaction Force Visualization During Oscillation",
	        aspect_ratio=:equal,
			opacity = 0.2
	    )
		...
		#Vertical cart reaction force
		quiver!(
    	[cart_position_x[index]],
    	[floor],
    	quiver = (
        	[0.0],
			[force_scale_v*cart_vertical_force[index]]
		),
			color = :green,
			linewidth = 4,
			label = false
		...
		# Horizontal hinge reaction force
		quiver!(
   		[beam_hinge_x[index]],
    	[beam_hinge_y[index]],
    	quiver = (
        	[force_scale_h * hinge_force_x[index]],
        	[0.0]
    	),
    		color = :red,
   			linewidth = 4,
		...
		# Vertical hinge reaction force
		quiver!(
    	[beam_hinge_x[index]],
   		[beam_hinge_y[index]],
    	quiver = (
       		[0.0],
        	[force_scale_v * hinge_force_y[index]]
    	),
    		color = :orange,
    		linewidth = 4,
			label = false
	...
	#Display magnitudes next to arrows
	ground_force_mag = abs(cart_vertical_force[index])
	hinge_fx_mag     = abs(hinge_force_x[index])
	hinge_fy_mag     = abs(hinge_force_y[index])
	annotate!(
    	cart_position_x[index] + 0.14,
    	floor + force_scale_v * cart_vertical_force[index] + 0.05,
    	text("$(round(ground_force_mag, digits=1)) N", 8, :green)
	)
	annotate!(
    	beam_hinge_x[index] + force_scale_h * hinge_force_x[index] + 0.05,
    	beam_hinge_y[index] - 0.05,
    	text("$(round(hinge_fx_mag, digits=1)) N", 8, :red)
	)
	annotate!(
    	beam_hinge_x[index] + 0.05,
    	beam_hinge_y[index] + force_scale_v * hinge_force_y[index] + 0.05,
    	text("$(round(hinge_fy_mag, digits=1)) N", 8, :orange)
	)

	
	annotation_x = x_max + 0.5
	annotation_y_top = force_max - 0.21
	annotation_dy = 0.07
	annotate!(
    	annotation_x,
    	annotation_y_top,
    	text("Peak Ground: $(round(peak_ground_force, digits=1)) N", 9, :green)
	)
	annotate!(
    	annotation_x,
    	annotation_y_top - annotation_dy,
    	text("Peak Hinge Fx: $(round(peak_hinge_fx, digits=1)) N", 9, :red)
	)
	annotate!(
    	annotation_x,
    	annotation_y_top - 2annotation_dy,
    	text("Peak Hinge Fy: $(round(peak_hinge_fy, digits=1)) N", 9, :orange)
	)
	annotate!(
    	x_max + 0.5,
   		force_max - 0.1,
    	text("Vertical Force Scale = 0.03\nHorizontal Force Scale = 0.08", 8)
	)

	end
end

# ╔═╡ cb49644c-1ef9-4bcd-8084-2026e4eeea04
gif(animation_force_display, "cart_beam_animation_with_trails.gif", fps=20)

# ╔═╡ 90201d9b-fff6-4b9c-b46c-b15babb776f8
md"""
### **Energy Conservation and Distribution Analysis**

To validate the physical integrity of the simulation, we analyze the system's total mechanical energy. In a lossless model, the sum of kinetic and potential energy should remain constant over time.

*   **Cart Kinetic Energy ($KE_{cart}$):** Energy resulting from the horizontal translation of the cart mass.
*   **Beam Kinetic Energy ($KE_{beam}$):** Rotational energy as the beam oscillates around the pivot.
*   **Beam Potential Energy ($PE_{beam}$):** Gravitational potential energy relative to the beam's lowest point.

The following animation tracks these components in real-time, visualizing the continuous transfer of energy between the two degrees of freedom.
"""

# ╔═╡ 81c554a6-18e1-42df-8665-fce3a7b6fac0
begin
    t_vals = solution.t
    
    # Pre-allocate component vectors
    T_cart = zeros(length(t_vals))
    T_beam = zeros(length(t_vals))
    V_spring = zeros(length(t_vals))
    V_grav = zeros(length(t_vals))
    ...
    for i in 1:length(t_vals)
        ...
        # Block 1 (x, y, θ)
        T_cart[i] = 0.5 * (m1 * v[1]^2 + m1 * v[2]^2 + I1 * v[3]^2)
        # Bar 2 (x, y, θ)
        T_beam[i] = 0.5 * (m2 * v[4]^2 + m2 * v[5]^2 + I2 * v[6]^2)
        
        # Potential Energy Components
        V_spring[i] = 0.5 * k * (q[1])^2
        V_grav[i]   = m1 * g * q[2] + m2 * g * q[5]
    end
    
    V_total = V_spring .+ V_grav
    E_total = T_cart .+ T_beam .+ V_total
end

# ╔═╡ 991862e0-c498-4a6e-9017-75b24d72f938
let
    # 1. PARAMETERS (Syncing with your model's constants)
    g_v  = 9.81
    L_v  = 0.4 
    mc_v = 0.1   
    mb_v = 0.3   
    ...
    #pos
    x_pos = [u[1] for u in U]
    θ_pos = [u[6] for u in U]

    #vels
    ẋ  = [0.0; diff(x_pos) ./ diff(t_ref)]
    θ̇ = [0.0; diff(θ_pos) ./ diff(t_ref)]

    #beam KE
    ẋ_cm = ẋ .+ (L/2) .* cos.(θ_pos) .* θ̇
    ẏ_cm = (L/2) .* sin.(θ_pos) .* θ̇
    v_cm2 = ẋ_cm.^2 .+ ẏ_cm.^2

    I_cm = (1/12) * mb_v * L^2
    ke_beam = 0.5 .* mb_v .* v_cm2 .+ 0.5 .* I_cm .* θ̇.^2

    #cart KE
    ke_cart = 0.5 .* mc_v .* ẋ.^2

    #beam PE
    pe_beam = mb_v .* g_v .* (L/2) .* (1 .- cos.(θ_pos))

    E_total = ke_beam .+ pe_beam .+ ke_cart
    ymax_val = maximum(E_total) * 1.3

    #animate
    anim = @animate for i in 1:5:length(t_ref)
        ...
        plot!(
            title="Energy Exchange: Filled KE ↔ PE Contrast",
            xlabel="Time (s)",
            ylabel="Energy (J)",
            xlim=(0, t_ref[end]),
            ylim=(0, ymax_val),
    ...
    gif(anim, "energy_filled_contrast.gif", fps=15)
end

# ╔═╡ b6d54564-c083-46a8-98a5-bd7db67bc538
md"""
### Analysis of System States and Coupling Dynamics

To evaluate the interaction between the cart's translation and the beam's angular displacement, the following visualization provides a comparative analysis of the system's behavior. 

* **Physical Coordinate Mapping (Left):** This plot renders the system in Cartesian coordinates ($x, y$), tracking the horizontal position of the cart and the resulting vertical trajectory of the beam. This serves as the primary verification for geometric constraints and kinematic consistency.
* **State-Space Phase Portrait (Right):** By plotting the cart position ($x$) against the beam angle ($\theta$), we can observe the system's "state-space" trajectory. This representation is critical for identifying the degree of coupling between the two degrees of freedom.
* **Energy Signatures:** The geometry of the phase portrait reveals the stability of the system; closed orbits indicate periodic motion, while complex or non-repeating paths highlight the nonlinear energy transfer between the cart and the beam.
"""

# ╔═╡ d6d4a195-f5ef-48b0-b1bf-96c6be2d8087
begin
    # Prepare data from solution
    c_x = solution[1, :] # Cart X
    b_θ = solution[6, :] # Beam Angle
    
    # Calculate tip coordinates for visualization
    t_x = c_x .+ L .* cos.(b_θ)
    t_y = solution[2, :] .+ L .* sin.(b_θ)

    x_l = (minimum(c_x) - 0.2, maximum(t_x) + 0.2)
    y_l = (minimum(t_y) - 0.1, maximum(t_y) + 0.1)

    animation_coupling = @animate for i in 1:2:length(t_vals)
        # SUBPLOT 1: Physical Motion
        p1 = plot([c_x[i], t_x[i]], [solution[2, i], t_y[i]],
            linewidth = 5, color = :blue,
            xlims = x_l, ylims = y_l,
            title = "Physical Motion (t = $(round(t_vals[i], digits=2))s)",
            aspect_ratio = :equal, legend = false)
        scatter!([c_x[i]], [solution[2, i]], color = :black, markersize = 6)

        # SUBPLOT 2: Phase Portrait (State-Space Coupling)
        p2 = plot(c_x[1:i], b_θ[1:i],
            xlabel = "Cart Position x (m)",
            ylabel = "Beam Angle θ (rad)",
            title = "Coupling: x vs θ",
            color = :purple, linewidth = 1.5,
            xlims = (minimum(c_x)-0.05, maximum(c_x)+0.05),
            ylims = (minimum(b_θ)-0.1, maximum(b_θ)+0.1),
            legend = false)
        scatter!([c_x[i]], [b_θ[i]], color = :purple, markersize = 4)

        plot(p1, p2, layout = (1,2), size=(900, 600))
    end
    
    gif(animation_coupling, "coupling_analysis.gif", fps = 60)
end

# ╔═╡ 818aa15e-53a1-47e8-a2f4-c1ddc59b70f3
begin
    beam_ang_vel_accel_anim = solution[12, :]

    beam_ang_accel_anim =
        diff(beam_ang_vel_accel_anim) ./ diff(time)

    beam_ang_accel_anim =
        [beam_ang_accel_anim; beam_ang_accel_anim[end]]

    accel_min_anim = minimum(beam_ang_accel_anim)
    accel_max_anim = maximum(beam_ang_accel_anim)

    animation_accel_full = @animate for i in 1:2:length(time_elapsed)

        level = (beam_ang_accel_anim[i] - accel_min_anim) /
                (accel_max_anim - accel_min_anim + 1e-8)

        beam_color_anim = cgrad(:plasma)[level]

        cart_x_anim, cart_y_anim = cart_vertices(
            cart_position_x[i],
            cart_position_y[i],
            block_w,
            block_h
        ...
        plot(
            ...
            xlabel = "X Position (m)",
            ylabel = "Y Position (m)",
            title = "Cart-Beam Motion Colored by Angular Acceleration",
            aspect_ratio = :equal,
            legend = false
        )

        # wheels
        wheel_center_x_positions_anim = begin
            usable_width_anim = block_w - 2wheel_r
            cart_center_x_anim = cart_position_x[i]
            cart_center_x_anim .+ [-usable_width_anim/2, 0.0, usable_width_anim/2]
        end

        wheel_center_y_anim = cart_position_y[i] - block_h/2 - wheel_r
        circle_angle_anim = range(0, 2π, length = 40)

        for wheel_center_x_anim in wheel_center_x_positions_anim
            plot!(
                wheel_center_x_anim .+ wheel_r .* cos.(circle_angle_anim),
                wheel_center_y_anim .+ wheel_r .* sin.(circle_angle_anim),
                seriestype = :shape,
                color = :lightgray,
                linewidth = 0.5,
                opacity = 1
        ...
        # floor
        plot!(
            [minimum(cart_position_x)-block_w/2, maximum(cart_position_x)+block_w/2],
            [floor, floor],
            color = :black,
            linewidth = 4,
            label = false
        ...
        # spring
        spring_left_x_anim  = x_wall
        spring_right_x_anim = cart_position_x[i] - block_w/2
        spring_y_anim       = cart_position_y[i]

        spring_x_anim, spring_y_vals_anim = spring_curve(
            spring_left_x_anim,
            spring_right_x_anim,
            spring_y_anim;
            n = 10,
        ...
        # beam
        plot!(
            [beam_hinge_x[i], beam_tip_x[i]],
            [beam_hinge_y[i], beam_tip_y[i]],
            linewidth = 8,
            color = beam_color_anim
        )
    ...
    gif(animation_accel_full, "slow_cart_beam_acceleration_colored.gif", fps = 16)
end
