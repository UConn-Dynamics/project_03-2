...
begin
    ...
    block_w = 0.12 # for plot, made up
    block_h = 0.08 # for plot, made up
...
# ╔═╡ f641595b-3d09-4888-baea-a875744fe40d
function Cqdot(q, qdot)
    beam_angle = q[6]
    beam_angular_velocity = qdot[6]

    return [qdot[2]
			qdot[3]
			qdot[1]-qdot[4]-(L/2)*sin(beam_angle)*beam_angular_velocity
			qdot[2]-qdot[5]+(L/2)*cos(beam_angle)*beam_angular_velocity
    	]
end
...
# ╔═╡ 87004f97-3dcf-4184-a5fb-54345fb25f3e
function q_external_matrix(generalized_position)
    cart_position_x = generalized_position[1]

    return [
        -k * cart_position_x   # cart x (spring)
        -m1*g                   # cart y (constrained)
        0.0                    # cart rotation (constrained)
        0.0                    # beam x
        -m2 * g                # beam y (gravity)
        0.0                    # beam rotation
    ]
...
begin
	function q_diff_constraints(q,qdot)
		...
		constraint_values = C(q)
    	constraint_velocities = Cqdot(q,qdot)

    	stabilization_alpha = 5.0
    	stabilization_beta = 5.0
		...
		return centripetal_terms .- 2*stabilization_alpha .* constraint_velocities .-stabilization_beta^2 .* constraint_values
...
# ╔═╡ b8ffc30f-9b75-4f9a-814c-f3b844bdfb17
differential_variables = vcat(
    trues(12),   # q and qdot
    falses(4)    # lambda (algebraic)
)
...
#Animation Parameter
begin
	x_min = minimum(beam_hinge_x) - 2*L
	x_max = maximum(beam_hinge_x) + 2*L
	
	y_min =  minimum(beam_tip_y) - L
	y_max =  maximum(beam_tip_y) + L
end

# ╔═╡ d28836b6-574a-4378-86ee-bfcc4bdb1423
begin
	animation = @animate for index in 1:5:length(time_elapsed)
	    plot(
	        ...
	        xlabel="X Position (m)",
	        ylabel="Y Position (m)",
	        title="Cart–Beam System Animation",
	        aspect_ratio=:equal,
	        legend=false
	    )
...
gif(animation, "cart_beam_animation.gif", fps=1000)
