...
# ╔═╡ 0a643f07-a650-47ff-8c5c-cefcc215363b
md"""
...
# ╔═╡ 4f1fd970-4854-456f-b37f-bcd3c9d62eb7

begin
    # index jump of 2 (instead of 5) slows down the animation for better viewing
    animation_phase = @animate for index in 1:2:length(time_elapsed)
        
        # SUBPLOT 1: Physical Motion
        # X-axis represents the horizontal displacement of the cart
        # Y-axis represents the vertical displacement of the beam components
        p1 = plot(
            [beam_hinge_x[index], beam_tip_x[index]],
            [beam_hinge_y[index], beam_tip_y[index]],
        ...
        # Representing the cart/hinge point
        scatter!([beam_hinge_x[index]], [beam_hinge_y[index]], color=:black, markersize=6)
        
        # SUBPLOT 2: Phase Portrait (Cart X vs Beam Angle)
        p2 = plot(
            cart_position_x[1:index], 
            beam_angle[1:index],
            xlabel="Cart Position (m)",
            ylabel="Beam Angle (rad)",
            title="Phase Portrait: X vs θ",
            color=:purple, linewidth=1.5,
            xlims=(minimum(cart_position_x)-0.05, maximum(cart_position_x)+0.05),
            ylims=(minimum(beam_angle)-0.2, maximum(beam_angle)+0.2),
            legend=false
        )
        # Current state indicator
        scatter!([cart_position_x[index]], [beam_angle[index]], color=:purple, markersize=4)

        plot(p1, p2, layout=(1,2), size=(900, 400), margin=5Plots.mm)
    end
end

# ╔═╡ 466f8d2b-069f-42a3-b5e5-e920012c9e44
gif(animation_phase, "phase_portrait_evolution.gif", fps=20)

# ╔═╡ 43b567da-4911-4eac-8ff0-1e831730cd80
md"""
...
# ╔═╡ 672eef26-df34-4c57-90bc-c90a98a151e4
let
    # 1. PARAMETERS
    g_v  = 9.81
    L_v  = 1.0
    ...
    sol_ref = @isdefined(sol) ? sol : (@isdefined(solution) ? solution : res)
    t_ref   = sol_ref.t
    U       = sol_ref.u
    ...
    ymax_val = maximum(ke_beam .+ pe_beam .+ ke_cart) * 1.3

    # 8. ANIMATION — FILLED BUT NOT STACKED
...
# ╔═╡ 643a0e99-8548-417e-be23-51c98b485637
begin
	...
	# 8. Animation
	anim = @animate for index in 1:5:length(time_elapsed)
	
	    # 8.1 System Motion - Top Plot
	    p1 = plot()
	    ...
	    plot!(p1, cart_x, cart_y, seriestype=:shape, color=:gray)
	
	    # Wheels
	    usable_width = block_w - 2*wheel_r
	    cart_center_x = cart_position_x[index]
	    wheel_center_x_positions = cart_center_x .+ [-usable_width/2, 0.0, usable_width/2]
	    wheel_center_y = cart_position_y[index] - block_h/2 - wheel_r
	    ...
	    plot!(p1, spring_x, spring_y_vals, linewidth=2, color=:purple)
	    ...
	    # Points
	    scatter!(p1, [beam_hinge_x[index]], [beam_hinge_y[index]], color=:black)
	    scatter!(p1, [beam_com_x[index]],   [beam_com_y[index]],   color=:red)
	    scatter!(p1, [beam_tip_x[index]],   [beam_tip_y[index]],   color=:green)
	
	    # Wall
	    plot!(p1,
	        [x_wall, x_wall],
	        [y10-2*block_h, y10+2*block_h],
	        color=:black,
	        linewidth=4
	    )
	    ...
	    # 8.2 Energy Progression - Bottom Plot
	    p2 = plot(
	        t_ref[1:index],
	        E_total[1:index],
	        fillrange=0,
	    ...
	    plot!(p2, t_ref[1:index], ke_beam[1:index],
	        fillrange=0, fillalpha=0.35, color=:green, label="Beam KE")
	
	    plot!(p2, t_ref[1:index], pe_beam[1:index],
	        fillrange=0, fillalpha=0.35, color=:orange, label="Beam PE")
	
	    plot!(p2, t_ref[1:index], ke_cart[1:index],
	        fillrange=0, fillalpha=0.25, color=:blue, label="Cart KE")
	
	    # Time marker line
	    plot!(p2,
	        [t_ref[index], t_ref[index]],
	        [0, ymax_val],
	        color=:black,
	        linestyle=:dash,
	        label=""
	    ...
	    # 8.3 Plot 
	    plot(p1, p2, layout=(2,1), size=(500,900))
	
	end
	
	gif(anim, "combined_animation.gif", fps=8)
end
